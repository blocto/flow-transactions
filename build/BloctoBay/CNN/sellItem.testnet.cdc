...
transaction(saleItemID: UInt64, saleItemPrice: UFix64) {
    ...
    let cnnNFTProvider: Capability<&CNN_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    prepare(signer: AuthAccount) {
        ...
        let cnnNFTCollectionProviderPrivatePath = /private/cnnNFTCollectionProviderForNFTStorefront
        if !signer.getCapability<&CNN_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(cnnNFTCollectionProviderPrivatePath)!.check() {
            signer.link<&CNN_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(cnnNFTCollectionProviderPrivatePath, target: CNN_NFT.CollectionStoragePath)
        }
        ...
        self.cnnNFTProvider = signer.getCapability<&CNN_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(cnnNFTCollectionProviderPrivatePath)!
        assert(self.cnnNFTProvider.borrow() != nil, message: "Missing or mis-typed CNN_NFT.Collection provider")
    ...
    execute {
        ...
        let id = self.storefront.createListing(
            nftProviderCapability: self.cnnNFTProvider,
